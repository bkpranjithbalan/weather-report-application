...
const CurrentWeatherCard = ({ data }: CurrentWeatherCardProps) => {
  ...
  const getTempColor = (temp: number) => {
    if (temp >= 35) return "text-temp-hot";
    if (temp >= 20) return "text-temp-warm";
    if (temp >= 10) return "text-temp-cool";
        ...
        <div className="relative flex flex-col md:flex-row items-center md:items-start gap-6">
          <div className="flex flex-col items-center md:items-start">
            ...
            <div className="flex items-start gap-1">
              <span className={`text-7xl md:text-8xl font-display font-bold text-glow ${getTempColor(current.temperature)}`}>
                {current.temperature}
              </span>
...
const DetailCard = ({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) => (
  <div className="glass rounded-2xl p-4 flex flex-col gap-2 hover:border-primary/20 transition-all duration-300 group">
